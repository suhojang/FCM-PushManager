...
function fn_detail(no){
	var frm	= document.frm;
	frm.APPINF_NO.value	= no;
	frm.action	= "/SEL_APP_001001.do";
	frm.submit();
}
...
<input type="hidden" name="APPINF_NO" id="APPINF_NO" value="">
			...
			<div class="bbs_info">
				...
				<div class="search">
					...
					<select class="TypeSelect" id="searchCondition" name="searchCondition">
						<option value="0">앱이름</option>
						<option value="1">패키지명</option>
					</select>
				...
				<caption>리스트</caption>
					...
					<c:if test="${!empty result}">
						...
						<c:set var="listSid" value="${listSid+1}"/>
							<tr onclick="javascript:fn_detail('${result.APPINFNO }');" style="cursor: pointer;">
								...
								<td style="width: 30%;text-align: center;">${result.APPINFAPPNM }</td>
								<td>${result.APPINFPKNM }</td>
								<td>${result.APPINFUSEYN }</td>
								<td>${result.APPINFRDT }</td>
							</tr>
						</c:forEach>
					</c:if>
					</tbody>
