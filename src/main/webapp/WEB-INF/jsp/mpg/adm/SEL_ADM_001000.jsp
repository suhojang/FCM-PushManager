...
function fn_detail(id){
	var frm	= document.frm;
	frm.ADMINF_ID.value	= id;
	frm.action	= "/SEL_ADM_001001.do";
	frm.submit();
}
...
<input type="hidden" name="ADMINF_ID" id="ADMINF_ID" value="">
				...
				<caption>리스트</caption>
					...
					<c:if test="${!empty result}">
						...
						<c:forEach items="${result}" var="result">
							...
							<tr onclick="javascript:fn_detail('${result.ADMINFID }');" style="cursor: pointer;">
								...
								<td style="width: 30%;text-align: center;">${result.ADMINFID }</td>
								<td>${result.ADMINFNM }</td>
								<td>${result.ADMINFDELYN }</td>
								<td>${result.ADMINFRUSR }</td>
								<td>${result.ADMINFRDT }</td>
							</tr>
						</c:forEach>
					</c:if>
					</tbody>
