...
function fn_detail(no){
	var frm	= document.frm;
	frm.IPMGR_NO.value	= no;
	frm.action	= "/SEL_IPM_001001.do";
	frm.submit();
}
...
<input type="hidden" name="IPMGR_NO" id="IPMGR_NO" value="">
				...
				<caption>리스트</caption>
					...
					<c:if test="${!empty result}">
						...
						<c:forEach items="${result}" var="result">
							...
							<tr onclick="javascript:fn_detail('${result.IPMGRNO }');" style="cursor: pointer;">
								...
								<td style="width: 30%;text-align: center;">${result.IPMGRNM }</td>
								<td>${result.IPMGRIP }</td>
								<td>${result.IPMGRUSEYN }</td>
								<td>${result.IPMGRRDT }</td>
							</tr>
						</c:forEach>
					</c:if>
					</tbody>
